...
class Player(pygame.sprite.Sprite):
    ...
    def run_dust_animation(self):
        if self.status == 'run' and self.on_ground:
            ...
            if self.facing_right:
                pos = self.rect.bottomleft - pygame.math.Vector2(6,10) #place the animation at bottom left of player when runing right
    ...
    def get_status(self):
        if self.direction.y < 0:
            self.status = 'jump'
        elif self.direction.y > 1: #avoid the animation glitching bc of gravity
            self.status = 'fall'
        else:
